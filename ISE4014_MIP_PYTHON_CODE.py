    ...
    def ASSPopt(self, FlightData):
      ...
      # define separation matrix
        ...
        for i in range(NumFlight):
            ...
            cpx.variables.add(obj=[1.0],lb=[0.0],ub=[BigM], types=["C"], names=[var_name])
        ...
        ###your input###
        for r in range(NumRun):
            for k in range(NumRun): 
                if (k != r) :
                    for i in range(NumFlight):
                        c6_var = []
                        c6_coef = []
                        c6_var.append(x_ir[i][r])
                        c6_coef.append(1)
                        c6_var.append(x_ir[i][k])
                        c6_coef.append(1)
                        cpx.linear_constraints.add(
                        lin_expr=[cplex.SparsePair(c6_var,c6_coef)], senses="E", rhs=[1])
            ...
            for i in range(NumFlight):
                for j in range(NumFlight):
                    if (j != i):
                        c5_var=[]
                        c5_coef=[]
                        c5_var.append(x_ir[i][r])
                        c5_coef.append(1)
                        c5_var.append(y_jir[j][i][r])
                        c5_coef.append(-1)
                        c5_var.append(y_jir[i][j][r])
                        c5_coef.append(-1)
                        c5_var.append(x_ir[j][r])
                        c5_coef.append(1)
                        cpx.linear_constraints.add(lin_expr=[cplex.SparsePair(c5_var,c5_coef)], senses="L", rhs=[1])
        ...
        for i in range(NumFlight): 
            ...
            C3_var.append(t_i[i])
            C3_coef.append(1)
            cpx.linear_constraints.add(lin_expr=[cplex.SparsePair(C3_var,C3_coef)], senses="G", rhs=[T_i[i]])
        ...
        for i in range(NumFlight): 
            ...
            C4_coef.append(-1)
            cpx.linear_constraints.add(
            lin_expr=[cplex.SparsePair(C4_var,C4_coef)], senses="G", rhs=[-1*T_i[i]])
